...
<body>
	...
	<div class="main-content">
		...
		<h3>在庫情報</h3>
		在庫ID：${inventory.inventoryID} <br> タイトル：${inventory.title} <br>
		出品者会員名：${inventory.userNameSeller} <br>
		会員：${inventory.userIDSeller} <br> 出品価格：${inventory.price} <br>
		在庫：${inventory.isinInventoryName} <br> 状態：${inventory.stateName}
		<br> コメント：${inventory.note} <br>
